...
async function login() {
  ...
  console.log('Token (first 30 chars):', body.token?.slice(0, 30) + '...');
  return body.token;
}

async function getTeamIntel(token) {
  ...
  let json = null;
  try { json = JSON.parse(txt); } catch (e) { console.error('Failed to parse JSON'); }
...
(async () => {
  ...
  if (token) await getTeamIntel(token);
})();
