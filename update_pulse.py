...
filepath = r"d:\gharpayy_arena-main\gharpayy_arena-main\src\routes\pulse.tsx"
...
# 2. State
state_old = """  const [dateFilter, setDateFilter] = useState<string>(todayISO());
  const [weekMode, setWeekMode] = useState(false);
  const [weekAnchor, setWeekAnchor] = useState<Date>(new Date());"""
state_new = """  const [dateRangeOption, setDateRangeOption] = useState<string>("today");
  const [customStart, setCustomStart] = useState<string>(todayISO());
  const [customEnd, setCustomEnd] = useState<string>(todayISO());"""
content = content.replace(state_old, state_new)

# 3. Date logic
date_logic_old = """  // Week days for the selected week
...
date_logic_new = """  const { startIso, endIso, isSingleDay } = useMemo(() => {
    const today = new Date();
...
# 4. entries
entries_old = """  const entries = useMemo(() => {
    if (weekMode) {
      const allEntries = getEntries({});
...
entries_new = """  const entries = useMemo(() => {
    const allEntries = getEntries({});
...
# 5. groupedEntries
groups_old = """    let result = Object.values(groups).map(empEntries => {
...
groups_new = """    let result = Object.values(groups).map(empEntries => {
      if (isSingleDay) {
        const eodEntry = empEntries.find(e => e.slot === "eod");
        const regularEntries = empEntries.filter(e => e.slot !== "eod");
        return {
...
# 7. UI inputs
ui_inputs_old = """              <div className="relative w-full md:w-auto">
                <input
                  type="date"
                  value={dateFilter}
                  onChange={(e) => {
...
ui_inputs_new = """              <div className="flex flex-col md:flex-row items-center gap-2 w-full md:w-auto">
...
# 8. weekMode panel
week_panel_regex = r"\{weekMode && \(\s*<div.*?Week Report.*?</div>\s*\)\}"
import re
# Wait, replacing regex across many lines is tricky. I'll just use string replacement on a known chunk.
week_panel_old = """            {weekMode && (
